...
import { Leaf, Award, Eye, Gift, TreePine } from 'lucide-react';
import { toast } from 'sonner';
import { NFTDetailModal } from '@/components/NFTDetailModal';
import { getContract } from '@/lib/blockchain/contract';
import { ethers } from 'ethers';
type CarbonCreditNFTContract = {
  mintNFT(to: string, name: string, description: string): Promise<unknown>;
};
  ...
  const handleClaimReward = async (reward: CarbonCreditNFT) => {
    try {
      ...
      if (!(window as any).ethereum) {
        toast.error("Please connect MetaMask to claim rewards");
        return;
      }
      /*
      // 2️⃣ Get provider + signer
      const provider = new ethers.BrowserProvider((window as any).ethereum);
      const signer: ethers.JsonRpcSigner = await provider.getSigner();
      const contract = getContract(signer) as any; // 👈 quick fix for typing

      // 3️⃣ Mint NFT on blockchain
      ...
      await tx.wait();
      */
      // 1️⃣ Connect to provider & signer
      const provider = new ethers.BrowserProvider((window as any).ethereum);
      const signer: ethers.Signer = await provider.getSigner();
      // 2️⃣ Get contract instance
      const contract = getContract(signer);

      // 3️⃣ Prepare NFT minting data
      const tx = await contract.mintNFT(
      walletAddress,              // recipient
      "ipfs://your_metadata_uri"  // tokenURI (string)
      );
      ...
      // 5️⃣ Update profile carbon credits
